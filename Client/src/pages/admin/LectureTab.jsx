...
const LectureTab = () => {
  ...
  const fileChangeHandler = async (e) => {
    ...
    if (file) {
        ...
        try {
            ...
            if (res.data.success && uploadData?.secure_url && uploadData?.public_id) {
                setUploadVideoInfo({
                ...
                toast.error("Upload failed or response malformed.");
            }
        } catch (error) {
  ...
  const editLectureHandler = async (e) => {
    ...
    // Kiểm tra nếu chưa upload xong video mà đã submit
    if (
      !uploadVideoInfo &&
      (!selectedLecture.videoUrl || !selectedLecture.publicId)
    ) {
    ...
    try {
      ...
      if (res.data.success) {
        const updatedLecture = res.data.lecture;
        const updatedLectures = lecture.map((l) =>
          l._id === updatedLecture._id ? updatedLecture : l
        );
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        <div className="mt-4">
          <Button
            disabled={loading || mediaProgress} // Ngăn submit nếu đang upload
